 ...
 * @author Admin
 */
@WebServlet(urlPatterns = {"/registration-detail"})
public class RegistrationDetailServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        Course cc = c.getRegistrationDetial(aid, cid);
        if (cc.getId() == 0) {
            request.getRequestDispatcher("registration-list").forward(request, response);
        } else {
            request.setAttribute("course", cc);
            request.setAttribute("n", cc.getChapters().size());
