    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            dataTableInstance = $('#coursesTable').DataTable({
                ajax: {
                    url: '/SE1735_G5/manage-blog',
                    type: 'GET',
                    dataType: 'json',
                    success: function (response) {
                        var listBlog = JSON.parse(response.listBlog);
                        console.log(listBlog);
                        dataTableInstance.clear().rows.add(listBlog).draw();
                    }
                },
                columns: [
                    {data: 'creator.email',
                    ...
                    },
                    {
                        ...
                        render: function (data, type, row) {
                            // Combine 'creator.firstName' and 'creator.lastName'
                            if (type === 'display') {
                    ...
                    },
                    {
                        ...
                        render: function (data, type, row) {
                            return '<div class="dropdown">' +
                                    ...
                                    + '  <button id="alert" data-id="' + row.id + "-" + row.creator.id + '" class="dropdown-item">Alert</button>'
            ...
            dataTableInstance.on('click', '#alert', function () {
                ...
                console.log(blogId);
                console.log(aid);
                var confirmed = confirm("Are you sure to alert this blog?");
                if (!confirmed) {
                    return false;
                } else {
                     showSuccessMessage('Alert blog succesfully', 'Sucess');
                    $.ajax({
            ...
            dataTableInstance.on('click', '#button-56', function () {
                ...
                console.log(currentPage);
                console.log(id);
                var confirmed = confirm("Are you sure to delete blog?");
                if (!confirmed) {
                    return false;
                } else {
                    $.ajax({
                        url: "/SE1735_G5/manage-blog?blID=" + id,
                        type: "delete",
                        success: function (response) {
                            filterButtonClickHandler();
                            showSuccessMessage('Remove blog succesfully', 'Sucess');
                            dataTableInstance.ajax.reload(function () {
                                dataTableInstance.page(currentPage).draw('page');
                            });
                        },
